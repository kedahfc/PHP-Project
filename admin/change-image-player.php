...
$sql="update player set Player_Image=:pimage where id=:imgid";
            ...
            <?php if($error)
              ...
              <?php } 
                ...
                <form class="form-horizontal" name="player" method="post" enctype="multipart/form-data">
								...
								<?php 
								$imgid=intval($_GET['imgid']);
								$sql = "SELECT Player_Image from player where id=:imgid";
								$query = $dbh -> prepare($sql);
								$query -> bindParam(':imgid', $imgid, PDO::PARAM_STR);
								$query->execute();
								...
								<label for="focusedinput" class="col-sm-2 control-label"> Player Image </label>
								<div class="col-sm-8">
								<img src="player-images/<?php echo htmlentities($result->Player_Image);?>" width="200">
								</div>
								</div>
