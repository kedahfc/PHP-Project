...
$sql="update product set Image=:pimage where id=:imgid";
            ...
            <?php if($error)
              ...
              <?php } 
                ...
                <form class="form-horizontal" name="product" method="post" enctype="multipart/form-data">
								...
								<?php 
								$imgid=intval($_GET['imgid']);
								$sql = "SELECT Image from product where id=:imgid";
								$query = $dbh -> prepare($sql);
								$query -> bindParam(':imgid', $imgid, PDO::PARAM_STR);
								$query->execute();
								...
								<div class="col-sm-8">
								<img src="product-images/<?php echo htmlentities($result->Image);?>" width="200">
								</div>
								</div>
