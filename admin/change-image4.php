...
move_uploaded_file($_FILES["image_4"]["tmp_name"],"product-images/".$_FILES["image_4"]["name"]);
$sql="update product set Image_4=:pimage4 where id=:imgid";
            ...
            <?php if($error)
              ...
              <?php } 
                ...
                <form class="form-horizontal" name="product" method="post" enctype="multipart/form-data">
								...
								<?php 
								$imgid=intval($_GET['imgid']);
								$sql = "SELECT Image_4 from product where id=:imgid";
								$query = $dbh -> prepare($sql);
								$query -> bindParam(':imgid', $imgid, PDO::PARAM_STR);
								$query->execute();
                ...
                {	?>	
								...
								<div class="col-sm-8">
								<img src="product-images/<?php echo htmlentities($result->Image_4);?>" width="200">
								</div>
								</div>
