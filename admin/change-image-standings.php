...
$pimage=$_FILES["standings_image"]["name"];
move_uploaded_file($_FILES["standings_image"]["tmp_name"],"standings-images/".$_FILES["standings_image"]["name"]);
$sql="update standings set Standings_Image=:pimage where id=:imgid";
            ...
            <?php if($error)
              ...
              <?php } 
                ...
                <form class="form-horizontal" name="standings" method="post" enctype="multipart/form-data">
								...
								<?php 
								$imgid=intval($_GET['imgid']);
								$sql = "SELECT Standings_Image from standings where id=:imgid";
								$query = $dbh -> prepare($sql);
								$query -> bindParam(':imgid', $imgid, PDO::PARAM_STR);
								$query->execute();
								...
								<label for="focusedinput" class="col-sm-2 control-label"> Standings Image </label>
								<div class="col-sm-8">
								<img src="standings-images/<?php echo htmlentities($result->Standings_Image);?>" width="200">
								</div>
								</div>
